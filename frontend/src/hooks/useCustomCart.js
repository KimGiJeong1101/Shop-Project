...
const useCustomCart = () => {
  ...
  useEffect(() => {
    // 부수 효과 함수를 정의합니다. 이 함수는 컴포넌트가 마운트될 때와 query.isSuccess, query.data 또는 changeMutation.isSuccess 값이 변경될 때마다 실행됩니다.
    if (query.isSuccess || changeMutation.isSuccess) {
      // 만약 쿼리가 성공하거나 변이 함수가 성공했을 경우에만 실행됩니다.
