...
const IndexPage = () => { // IndexPage 컴포넌트 선언
    ...
    return (
        <BasicLayout> {/* BasicLayout 컴포넌트를 렌더링 */}
            ...
            {/* 리스트와 추가 버튼을 담는 div */}
            <div className="w-full flex justify-end m-2 p-2 pr-20"> {/* pr-60 대신 pr-0으로 수정 */}
                {/* 추가 버튼 */}
                {canNoticeAdd && (
                    <div className="inline-flex items-center border rounded-md bg-gray-50 px-3 py-2 text-sm shadow-sm hover:bg-gray-100" onClick={handleClickAdd}>
                        추가
                    </div>
                )}
            </div>
