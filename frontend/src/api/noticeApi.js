...
export const getList = async (pageParam) => {
  ...
  const res = await axios.get(`${host}/list`, {
    //axios.get() 함수를 호출. 첫 번째 매개변수는 요청을 보낼!!! URL
    //두 번째 매개변수는 옵션 객체. 여기서는 params 속성을 사용하여 쿼리 매개변수를 설정
    params: { page: page, size: size }, //params 객체 내부에는 서버에 전달할 쿼리 매개변수가 포함. 쿼리 매개변수를 설정
  });
...
export const putOne = async (nno, notice) => {
  // (pno, notice) 이건 단지 파라미터 2개를 받는다는 말
  const header = { headers: { "Content-Type": "multipart/form-data" } };
