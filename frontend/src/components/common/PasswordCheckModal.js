...
const PasswordCheckModal = ({
  ...
  const handleConfirmPw = () => {
    const selectPassword = listData.dtoList.find(
      (post) => post.ano === selectAno
    );
  ...
  return (
    ...
    >
      <div className="absolute bg-white shadow dark:bg-gray-700 opacity-100 w-1/4 rounded  mt-10 mb-10 px-6 min-w-[600px]">
        <div className="justify-center bg-warning-400 mt-6 mb-6 text-2xl border-b-4 border-white text-white"></div>

        <div className="pl-6">
          <input
            className="text-4xl  border-orange-300 border-b-4 pt-1 pb-1"
            name="password"
            type="password"
            placeholder="비밀번호"
            value={userPassword}
