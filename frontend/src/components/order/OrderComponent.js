...
const OrderComponent = () => {
  ...
  const [localDetailAddress, setLocalDetailAddress] = useState(
  ...
  const handleClickOrder = async () => {
    try {
      ...
      // 주문 정보를 DB에 넘김
      console.log(
        "전달된 아이템의 개수가 몇개인지 콘솔에 표시하면" + orders.length
      );
      if (orders.length === 1) {
          ...
          <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex-grow">
            ...
            <div className="border rounded my-4 p-6 ">
              <div className="sm:col-span-5 mb-4 ml-5 mt-2 flex">
                <label
                  htmlFor="name"
                  className="block text-sm font-bold leading-6 text-gray-900 w-20"
                >
                  받는사람 *
                </label>
              ...
              <div className="col-span-full mb-4 ml-5 flex">
                ...
                <button
                  className="items-center border rounded-md ml-5 bg-gray-50 px-2 py-0.5 text-sm shadow-sm hover:bg-gray-100"
                  onClick={handleAddressSearch}
                >
                  주소 입력
                </button>
                  ...
                  <div className="flex">
                    <div className="w-100 h-56 overflow-hidden">
                      {order.images && order.images.length > 0 ? (
                        <img
                          src={`${host}/api/products/view/${order.images[0]}`}
                          alt="Product"
                          className="thumbnail-image cursor-pointer"
                          onClick={() =>
                            (window.location.href = `/products/read/${order.productPno}`)
                          }
                        />
                      ) : (
                    ...
                    <div className="mt-4 ml-5">
                      ...
                      <p>
                        합산 가격 : {formatPrice(order.price * order.count)}원
                      </p>
                    </div>
                  </div>
                </div>
