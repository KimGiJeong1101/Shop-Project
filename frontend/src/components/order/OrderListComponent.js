    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {data &&
        data.content.map((order) => (
          <div
            key={order.orderId}
            className="bg-white border shadow overflow-hidden rounded-lg mb-6 p-6"
          >
            ...
            <div className="grid grid-cols-2 gap-2  text-sm">
              ...
              <p>수령인: {order.dname}</p>
              <p>
                배송 주소: {order.deliveryAddress} {order.detailAddress}
              </p>
              <p>연락처: {order.tel}</p>
              ...
              <>
                {showAllItems
                  ? order.orderItemDtoList.map((item, index) => (
                      ...
                      >
                        <div className="flex items-center">
                          <img
                            src={`${API_SERVER_HOST}/api/products/view/${item.uploadFileNames}`}
                            alt="Product"
                  ...
                  : order.orderItemDtoList.slice(0, 1).map((item, index) => (
                      <div
                        key={index}
                        className="flex justify-between items-center p-3 last:border-b-0"
                      >
                        <div className="flex items-center">
                          <img
                            src={`${API_SERVER_HOST}/api/products/view/${item.uploadFileNames}`}
                ...
                {order.orderItemDtoList.length > 1 && (
                  <div className=" m-4">
                    <button className="text-base" onClick={toggleShowAllItems}>
                      {showAllItems ? (
                        <>
                          접기 <ChevronUpIcon className="w-6 h-6 inline" />
                        </>
                      ) : (
                        <>
                          더보기 <ChevronDownIcon className="w-6 h-6 inline" />
                        </>
                      )}
                    </button>
                  </div>
